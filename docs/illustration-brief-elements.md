...
## 8. Итог приёмки первой партии

Партия из шести файлов принята частично. Проверялось не по кадру, а по
отрендеренной странице: элемент, который хорош в кадре 1024 px, на
полосе занимает 11–13 мм, и решает только это.

### Принято

**`e-yacht-side`** — работает, стоит на схеме якорной стоянки.
Замечание на будущее: мачта обрезана верхним краем кадра, задание
требовало поля вокруг объекта. На этом размере не мешает, но в следующей
партии кадрировать целиком.

### Отклонено: слишком детально для 11–13 мм

**`e-boat-close-hauled`, `e-boat-beam-reach`, `e-boat-broad-reach`,
`e-boat-running`.** В кадре хороши. На странице палубные люки, лебёдки,
кокпит, ванты и контур паруса сливаются в серую крошку, и схема читается
хуже, чем прежний плоский силуэт.

Увеличить нельзя: на круге курсов соседние лодки стоят в 13,8 мм друг от
друга, длина больше ~14 мм — и они начнут пересекаться.

Что изменить в промпте:

```
Plain hull outline only. No deck hardware, no winches, no hatches, no cleats,
no stanchions, no lifelines, no cabin windows, no rigging wires. Simple
uncluttered silhouette readable at thumbnail size: hull outline, mast as a
single short line, boom as a single line, sails as plain flat shapes with a
clean outline. Thick confident contour, sparse interior detail.
```

Проверка перед сдачей партии: уменьшите файл до 130 px по длинной стороне
и посмотрите. Примерно так он и будет выглядеть на бумаге. Если силуэт
перестал читаться — деталей всё ещё много.

**`e-vessel-power`** сам по себе читается и принят технически, но в книгу
не поставлен: он живёт на одной полосе с парусными схемами, и пока те
остаются векторными, страница выглядела бы собранной из двух разных книг.
Файл вернётся в работу вместе с перерисованными парусными элементами.

### Отличать бакштаг от галфвинда

В присланной партии паруса на этих двух отличаются градусов на десять.
На схеме они стоят рядом, и разница должна читаться сразу: галфвинд —
парус примерно на 45° от оси, бакштаг — на 70°, почти поперёк корпуса.
